...
describe('Flashcards empty state', () => {
  it('shows a message when no sets exist', () => {
    render(<Flashcards sets={[]} setSets={() => {}} />);

    expect(
      screen.getByText(/No flashcard sets yet/i)
    ).toBeInTheDocument();
  });
...
describe('Flashcards rendering', () => {
  it('renders the heading', () => {
    render(<Flashcards sets={testSets} setSets={() => {}} />);
  ...
  it('renders all set names', () => {
    ...
    expect(
      screen.getAllByText('Biology 101')[0]
    ).toBeInTheDocument();

    expect(
      screen.getByText('Math 120')
    ).toBeInTheDocument();
  });
  ...
  it('shows the first flashcard by default', () => {
    ...
    expect(
      screen.getByText('What is a cell?')
    ).toBeInTheDocument();

    expect(
      screen.getByText('The basic unit of life')
    ).toBeInTheDocument();
  });
  ...
  it('switches sets when clicked', () => {
    ...
    expect(
      screen.getByText('What is pi?')
    ).toBeInTheDocument();

    expect(
      screen.getByText('3.14159')
    ).toBeInTheDocument();
  });
  ...
  it('shows empty state for sets with no cards', () => {
    ...
    render(<Flashcards sets={emptySet} setSets={() => {}} />);
...
describe('Adding flashcards', () => {
  it('opens the add card form', () => {
    ...
    expect(
      screen.getByPlaceholderText('Type an answer')
    ).toBeInTheDocument();
  });
  ...
  it('saves a new flashcard', () => {
    ...
    const updatedSets =
      mockSetSets.mock.calls[0][0];

    const updatedSet = updatedSets.find(
      s => s.id === 1
    );

    const newCard = updatedSet.cards.find(
      c => c.q === 'What is mitosis?'
    );
  ...
  it('closes the add form on cancel', () => {
    ...
    expect(
      screen.queryByPlaceholderText('Type a question')
...
describe('Deleting flashcards', () => {
  it('deletes a flashcard', () => {
    ...
    fireEvent.click(
      screen.getAllByTitle('Delete card')[0]
    );
    ...
    expect(
      updatedSet.cards.find(c => c.id === 10)
    ).toBeUndefined();
  });
});
...
describe('Editing flashcards', () => {
  it('opens edit form with existing values', () => {
    ...
    fireEvent.click(
      screen.getAllByTitle('Edit card')[0]
    );
  ...
  it('saves edited flashcard text', () => {
    ...
    expect(updatedCard.q).toBe(
      'Updated question'
    );

    expect(updatedCard.a).toBe(
      'Updated answer'
    );
  });
